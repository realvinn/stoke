...
STOKE_PS1_LINE='powershell -ExecutionPolicy Bypass -c "irm https://stoke.vinn.dev | iex"'
# What a Windows shell running THIS script is handed over to (windows_handoff).
# The readable path rather than the bare host: nothing a proxy does to the
# User-Agent can turn it into the page or into this script again.
STOKE_PS1_URL='https://stoke.vinn.dev/install.ps1'
...
# A Windows shell that can run this script at all: Git Bash and MSYS2 (MINGW*,
# MSYS*), Cygwin, and anything else that exports Windows' own `OS=Windows_NT`
# into the environment. WSL is Linux and is deliberately not one of these.
on_windows() {
...
# The PowerShell to hand over to. Windows PowerShell 5.1 first: it ships in
# every Windows 10 and 11, and install.ps1 is written for it and for 7 alike.
windows_powershell() {
...
# Run the Windows installer instead of refusing. This used to print "run this
# in PowerShell instead" and exit 1, which sent somebody who had typed the
# macOS/Linux line into Git Bash off to find a second terminal for a line they
# could have been handed. Their environment (STOKE_DRY_RUN, STOKE_FORCE,
# NO_COLOR) goes with them, because a child inherits it.
#
# stdin is /dev/null, not the pipe: this script is still being read from curl's
...
windows_handoff() {
  ...
  die 'This is the macOS and Linux installer, and this looks like Windows.' \
    'Run this in Command Prompt or PowerShell instead:' \
    '' \
    "    $STOKE_PS1_LINE"
}
...
do_preflight() {
  ...
  if on_windows; then pf_platform=windows; fi
  # What main would do on Windows: run the PowerShell installer, or -- with no
  # PowerShell on PATH at all -- print the line and stop.
  pf_handoff=none
  if [ "$pf_platform" = windows ]; then
    if windows_powershell >/dev/null; then pf_handoff=powershell; else pf_handoff=message; fi
  fi
  pf_arch=unsupported
  case "$pf_machine" in
...
main() {
  ...
  # so stoke.vinn.dev hands it THIS script. Hand over to the one that runs there.
  if on_windows; then windows_handoff; fi
