 ...
 */
/**
 * Every agent id, wrapped under the `--cli` line. Generated from the table, so
 * a new agent reaches the help by being added there — and verify:stoke-args
 * then fails until the three shell copies of this text are regenerated.
 */
function agentIdLines(): string[] {
  const out: string[] = []
  let cur = ''
  for (const id of CODING_CLIS.map((c) => c.id)) {
    const next = cur ? `${cur}, ${id}` : id
    if (cur && next.length > 60) {
      out.push(`${cur},`)
      cur = id
    } else cur = next
  }
