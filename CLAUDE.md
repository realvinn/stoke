...
worklog-retry, worklog-recall, worklog-autoscan, ssh, remote, selection — the `check` chain — plus
extract and security, which
...
**The installer's campfire** — `.claude/rules/installer.md`
- **67.** Keep the installer art inside `campfire.ts`'s `ALPHABET` and generated by
